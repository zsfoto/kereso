...
class PersonsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		if($sort !== null && $direction !== null){
			...
			$this->paginate['Persons']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->Persons->find()->contain(['Icons', 'Companies'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		try {
			...
			$this->paginate['Persons']['maxLimit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		try {
			$person = $this->Persons->get((int) $id, contain: ['Icons', 'Companies']);
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
			...
			/*
				if(...){
					$person->setErrors('field', __('Message'));
				}
			*/
			//dd($person->getErrors());
            if (!$person->hasErrors() && $this->Persons->save($person)) {
        ...
        $icons = $this->Persons->Icons->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $companies = $this->Persons->Companies->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('person', 'icons', 'companies'));
    }
    ...
    public function edit($id = null)
    {
		...
		try {
			$person = $this->Persons->get((int) $id, contain: []);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$person->setError('field', __('Message'));
				}
			*/
			//dd($person->getErrors());
			if (!$person->hasErrors() && $this->Persons->save($person)) {
        ...
        $companies = $this->Persons->Companies->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $person->name;
        $this->set(compact('person', 'icons', 'companies', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
		...
		try {
			$person = $this->Persons->get((int) $id);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}
