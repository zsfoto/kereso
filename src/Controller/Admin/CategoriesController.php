...
class CategoriesController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Categories'));
		...
		$this->paginate['Categories']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['Categories']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['Categories']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->Categories->find()->contain(['Icons'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Categories']['limit'] = $this->config['paginate_limit'];
			$this->paginate['Categories']['maxLimit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		try {
			$category = $this->Categories->get((int) $id, contain: ['Icons', 'Companies']);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('View the') . ': ' . __('category') . ' ' . __('record'));
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('category') . ' ' . __('record'));
        ...
        if ($this->request->is('post')) {
			...
			/*
				if(...){
					$category->setErrors('field', __('Message'));
				}
			*/
			//dd($category->getErrors());
            if (!$category->hasErrors() && $this->Categories->save($category)) {
                //$this->Flash->success(__('The category has been saved.'), ['plugin' => 'JeffAdmin5']);
            ...
            //$this->Flash->error(__('The category could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $icons = $this->Categories->Icons->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('category', 'icons'));
    }
    ...
    public function edit($id = null)
    {
		...
		try {
			$category = $this->Categories->get((int) $id, contain: []);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('Edit the') . ': ' . __('category') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$category->setError('field', __('Message'));
				}
			*/
			//dd($category->getErrors());
			if (!$category->hasErrors() && $this->Categories->save($category)) {
        ...
        $icons = $this->Categories->Icons->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $category->name;
        $this->set(compact('category', 'icons', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
		...
		if ($this->Categories->delete($category)) {
			...
			//$this->Flash->success(__('The category has been deleted.'), ['plugin' => 'JeffAdmin5']);
			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
		} else {
			//$this->Flash->error(__('The category could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
