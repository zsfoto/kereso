...
<?php } ?>
											<td class="string link country-id" value="<?= $county->country_id ?>"><?= $county->hasValue('country') ? $this->Html->link($county->country->name_hu, ['controller' => 'Countries', 'action' => 'view', $county->country->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span></td>
											<td class="string name" value="<?= $county->name ?>"><?= h($county->name) ?></td>
											<td class="string shortname" value="<?= $county->shortname ?>"><?= h($county->shortname) ?></td>
